...
def test_snippet_respects_max_len():
    content = "a" * 100 + " LONGTOKEN " + "b" * 200
    snippet = _extract_snippet(content, ["longtoken"], max_len=200)
    # The inner slice (excluding "..." markers) should not exceed max_len
    inner = snippet.lstrip(".").rstrip(".")
...
def test_max_matches_limits_snippets_but_counts_score():
    with tempfile.TemporaryDirectory() as td:
        ...
        assert result["score"] == 9.0  # 3 user matches x weight 3
...
def test_search_session_indexes_summary():
    with tempfile.TemporaryDirectory() as td:
        ...
        p = _write_session(tmp, "sessSum", [
            {"type": "summary", "summary": "Fixed DynamoDB billing issue"},
        ])
        result = search_session(p, "DynamoDB", 3)
        assert result is not None
